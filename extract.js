            ...
            for(var i=0; i<total/15+1;i++){
                $.get("/people/" + userid + "/"+type+"?start="+(i*15), function(dat){
                    $(".item",dat).each(function(){
                        ...
                        comment=$("li:nth-child(4)",this).text().replace(/\s+/g, " ");comment=/修改\s删除/.test(comment)?" ": comment;
                        level=parseInt($("li:nth-child(3) span:first",this).attr("class").split(/rating|-/)[1]);textarea.append("链 接：【" + href+"】,个人评分：【" + level + "】, 片名：【" + title + "】, 个人短评：【" + comment + "】\n");
                    });
                })
            }
        });
